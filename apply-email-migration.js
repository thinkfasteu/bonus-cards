...
async function checkAndApplyMigration() {
    ...
    `);
        ...
        if (!hasAttempts) {
            await db_1.pool.query('ALTER TABLE email_receipts ADD COLUMN attempts INT NOT NULL DEFAULT 0');
            console.log('✅ Added attempts column');
        }
        if (!hasSentAt) {
            await db_1.pool.query('ALTER TABLE email_receipts ADD COLUMN sent_at TIMESTAMP WITH TIME ZONE NULL');
            console.log('✅ Added sent_at column');
        }
        if (!hasLastError) {
            await db_1.pool.query('ALTER TABLE email_receipts ADD COLUMN last_error TEXT NULL');
            console.log('✅ Added last_error column');
        }
        // Add indexes
        await db_1.pool.query('CREATE INDEX IF NOT EXISTS idx_email_receipts_status_created ON email_receipts(status, created_at)');
        await db_1.pool.query('CREATE INDEX IF NOT EXISTS idx_email_receipts_status_id ON email_receipts(status, id)');
        ...
        if (!hasMessageId) {
            await db_1.pool.query('ALTER TABLE email_receipts ADD COLUMN message_id TEXT NULL');
