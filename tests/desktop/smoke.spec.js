...
test_1.test.beforeAll(async () => {
    ...
    electronApp = await playwright_1._electron.launch({
        args: [path_1.default.join(__dirname, '../../dist/electron/main.js')],
        env: {
            NODE_ENV: 'test',
...
test_1.test.describe('Desktop App Smoke Tests', () => {
    (0, test_1.test)('should load login screen', async () => {
        // Check if login screen is visible
        await (0, test_1.expect)(page.locator('h1')).toContainText('FTG Sportfabrik');
        await (0, test_1.expect)(page.locator('input#username')).toBeVisible();
        // Check if the input field has focus
        const focusedElement = await page.evaluate(() => document.activeElement?.id);
        (0, test_1.expect)(focusedElement).toBe('username');
    });
    (0, test_1.test)('should show error for invalid login', async () => {
        ...
        await (0, test_1.expect)(page.locator('.error-message')).toContainText('nicht gefunden');
    });
    (0, test_1.test)('should login successfully with valid credentials (mocked)', async () => {
        ...
        await (0, test_1.expect)(submitButton).not.toBeDisabled();
        // Note: In a real test, we would mock the API response here
        // and verify navigation to the reception screen
    });
    (0, test_1.test)('should auto-focus scan input in reception screen (simulated)', async () => {
    ...
    (0, test_1.test)('should handle window controls', async () => {
        // Test basic window functionality
        const title = await page.title();
        (0, test_1.expect)(title).toContain('Digital Bonus Cards');
        // Check that the window is visible and has the right dimensions
        const bounds = await electronApp.evaluate(({ BrowserWindow }) => {
            const win = BrowserWindow.getAllWindows()[0];
            return win.getBounds();
        });
        (0, test_1.expect)(bounds.width).toBeGreaterThanOrEqual(800);
        (0, test_1.expect)(bounds.height).toBeGreaterThanOrEqual(600);
    });
    (0, test_1.test)('should load CSS and render properly', async () => {
        // Check that styles are loaded
        const hasStylesheet = await page.evaluate(() => {
            const stylesheets = document.styleSheets;
            return stylesheets.length > 0;
        });
        (0, test_1.expect)(hasStylesheet).toBe(true);
        // Check that the main app container exists
        await (0, test_1.expect)(page.locator('.app, .screen')).toBeVisible();
    });
});
// Mock API test - would be used with a test server
test_1.test.describe('API Integration (Mocked)', () => {
    test_1.test.skip('should handle API errors gracefully', async () => {
        // This would test API error handling with a mock server
        // Set up mock API responses for various error scenarios
    });
    test_1.test.skip('should process card scan successfully', async () => {
