...
const sendReceipt_1 = require("../src/mailer/sendReceipt");
const transport_1 = require("../src/mailer/transport");
const emailWorker_1 = require("../src/worker/emailWorker");
const db_1 = require("../src/db");
const nodemailer_mock_1 = __importDefault(require("nodemailer-mock"));
/**
 * Email System Tests
 *
...
describe('Email System', () => {
    beforeEach(() => {
        ...
        nodemailer_mock_1.default.mock.reset();
    ...
    describe('SMTP Transport', () => {
        test('should create transport in dry-run mode', () => {
            const transport = (0, transport_1.createSMTPTransport)({
                host: 'localhost',
                port: 587,
                secure: false,
                user: 'test',
        ...
        test('should handle health check in dry-run mode', async () => {
            const result = await (0, transport_1.healthCheckSMTP)();
        ...
        test('should fail health check with invalid SMTP config', async () => {
            ...
            const result = await (0, transport_1.healthCheckSMTP)(transport);
    ...
    describe('Receipt Sending', () => {
        test('should validate receipt payload correctly', () => {
            ...
            const validation = (0, sendReceipt_1.validateReceiptPayload)(validPayload);
        ...
        test('should reject invalid payload', () => {
            ...
            const validation = (0, sendReceipt_1.validateReceiptPayload)(invalidPayload);
    ...
    describe('Email Worker', () => {
        ...
        beforeEach(() => {
            ...
            worker = new emailWorker_1.EmailWorker(db_1.pool, {
                batchSize: 5,
                concurrency: 1,
                maxRetries: 2,
      ...
      `);
            ...
            const queuedEmails = await db_1.pool.query(`
        SELECT 
      ...
      `, [emailId, result.messageId]);
            ...
            const updatedEmail = await db_1.pool.query(`
      ...
      `, [emailId]);
            ...
            // Clean up test data
            await db_1.pool.query('DELETE FROM email_receipts WHERE id = $1', [emailId]);
        });
    });
    describe('Email Template Generation', () => {
        ...
        test('should include remaining uses for bonus cards', async () => {
            ...
            await (0, sendReceipt_1.sendDeductionReceipt)({
