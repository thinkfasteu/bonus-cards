...
exports.generateDeductionEmailContent = generateDeductionEmailContent;
exports.generateRollbackEmailContent = generateRollbackEmailContent;
const db_1 = require("../db");
async function enqueueEmail(toEmail, subject, bodyText, bodyHtml) {
    ...
    const result = await (0, db_1.query)(`INSERT INTO email_receipts (to_email, status, created_at)
     VALUES ($1, 'Queued', now())
     RETURNING id`, [toEmail]);
    return result.rows[0].id;
}
// Email templates in German (user-facing)
function generateDeductionEmailContent(memberDisplayName, cardSerial, product, remainingUses, deductedAt) {
...
function generateRollbackEmailContent(memberDisplayName, cardSerial, product, remainingUses, rolledBackAt, reasonCode) {
    const productName = product === 'cycling_bonus' ? 'Bonus-Karte Fahrrad' : 'Unlimited-Karte Fahrrad';
