...
/**
 * Email Receipt Sender for FTG Sportfabrik Digital Bonus Cards
 * 
 * Sends German-language transactional receipts for card deductions.
...
export interface ReceiptPayload {
  ...
  serial: string;
  eventTimeUTC: Date;
  remainingUses: number | null; // null for unlimited products
  expiresAtUTC: Date;
}

export interface SendReceiptOptions {
  to: string;
  locale?: string; // Currently only 'de' supported
  payload: ReceiptPayload;
  transport?: Transporter; // Optional for testing
}
 ...
 */
function formatBerlinTime(utcDate: Date, includeDate: boolean = true): string {
 ...
 */
function formatBerlinDate(utcDate: Date): string {
 ...
 */
function generateSubject(productLabel: string): string {
 ...
 */
function generateTextBody(payload: ReceiptPayload): string {
 ...
 */
function generateHtmlBody(payload: ReceiptPayload): string {
  const localTime = formatBerlinTime(payload.eventTimeUTC, false);
  const expiresLocal = formatBerlinDate(payload.expiresAtUTC);
 ...
 */
export async function sendDeductionReceipt(options: SendReceiptOptions): Promise<SendReceiptResult> {
  ...
  try {
    ...
    const emailTransport = transport || getTransport();
 ...
 */
export function validateReceiptPayload(payload: ReceiptPayload): { valid: boolean; errors: string[] } {
  const errors: string[] = [];
