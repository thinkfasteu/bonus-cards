 ...
 */
router.get('/email-receipts', async (req, res) => {
    try {
        ...
        let whereClause = '';
        const queryParams = [limitNum, offsetNum];
        if (status && typeof status === 'string') {
            const validStatuses = ['Queued', 'Sent', 'Failed'];
            if (validStatuses.includes(status)) {
                ...
                return res.status(400).json({
                    error: 'Invalid status. Must be one of: Queued, Sent, Failed'
                });
            }
        }
 ...
 */
router.post('/email-receipts/retry/:id', async (req, res) => {
    try {
        const { id } = req.params;
      ...
      WHERE id = $1
    `;
        const checkResult = await db_1.pool.query(checkQuery, [receiptId]);
    ...
    `;
        const retryResult = await db_1.pool.query(retryQuery, [receiptId]);
        const updatedReceipt = retryResult.rows[0];
        console.log(`📧 Admin retry: Receipt ${receiptId} reset to Queued by ${req.headers['x-staff-username']}`);
        res.json({
            success: true,
            receipt: {
 ...
 */
router.get('/email-receipts/stats', async (req, res) => {
    try {
        const query = `
      SELECT 
        status,
 ...
 */
router.get('/email-receipts/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const receiptId = parseInt(id, 10);
        if (isNaN(receiptId)) {
      ...
      WHERE er.id = $1
    `;
        const result = await db_1.pool.query(query, [receiptId]);
